#!/bin/bash

echo "=== Диагностика миграций на сервере ==="

echo "1. Проверяем статус миграций..."
sudo docker compose exec backend python manage.py showmigrations core

echo ""
echo "2. Проверяем структуру таблицы core_pricelisttask..."
sudo docker compose exec db psql -U postgres -d autoparts -c "\d core_pricelisttask"

echo ""
echo "3. Проверяем структуру таблицы core_parsingtask..."
sudo docker compose exec db psql -U postgres -d autoparts -c "\d core_parsingtask"

echo ""
echo "4. Проверяем логи backend с ошибками..."
sudo docker compose logs backend --tail=50 | grep -A 5 -B 5 "ERROR\|Exception\|Traceback"

echo ""
echo "5. Проверяем статус контейнеров..."
sudo docker compose ps
